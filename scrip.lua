...
function AddESPForPlayer(pl)
    ...
    -- Remove stale ESP first so respawns/new players are handled correctly.
    ...
    if not hum or not root or not head then
        task.spawn(function()
            local newChar = pl.Character or pl.CharacterAdded:Wait()
            if not Cheat.Flags.ESP or not newChar then return end
            task.wait(0.2)
            if pl.Character == newChar then AddESPForPlayer(pl) end
        end)
        return
    end
    ...
    Cheat.Runtime.ESPSmoothed[pl] = {root = root.Position, head = head.Position}

    if Cheat.Runtime.ESPCharacterConnections[pl] then
        pcall(function() Cheat.Runtime.ESPCharacterConnections[pl]:Disconnect() end)
    end
    Cheat.Runtime.ESPCharacterConnections[pl] = pl.CharacterAdded:Connect(function(newChar)
        if not Cheat.Flags.ESP then return end
        task.wait(0.25)
        if pl.Parent and pl.Character == newChar then
            AddESPForPlayer(pl)
        end
    end)
end
...
function UpdateESP()
    ...
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer then
            ...
            -- ESP не исчезает при смерти: Humanoid остаётся в персонаже до его удаления.
            -- Показываем 0 HP и продолжаем считать дистанцию до трупа.
            if hum and root and head then
                ...
                -- Персонаж действительно удалён между смертью и респавном.
                -- Не удаляем PlayerAdded/CharacterAdded connection: новый ESP создастся после респавна.
...
Players.PlayerAdded:Connect(function(pl)
    ...
    task.spawn(function()
        ...
        if not Cheat.Flags.ESP or not pl.Parent then return end
        task.wait(0.25)
        if pl.Character == char then
            AddESPForPlayer(pl)
        end
    end)
end)
...
Cmds.help = {
    ...
    run = function(args, output)
        ...
        local commands = {
            "help", "fly", "unfly", "speed", "noclip", "fakeheal", "goto/tp", "esp", "saitama",
...
local function StopFly()
    Cheat.Flags.Fly = false
    if Cheat.Runtime.FlyBodyVelocity then
        pcall(function() Cheat.Runtime.FlyBodyVelocity:Destroy() end)
        Cheat.Runtime.FlyBodyVelocity = nil
    end
    local char = LocalPlayer.Character
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then hum.PlatformStand = false end
    if IsMobile and JoyGui and JoyGui.Parent then JoyGui.Enabled = false end
end

Cmds.fly = {
    desc = "Полет: fly [скорость] (до 1e19). Выключение: unfly",
    run = function(args, output)
        local requested = tonumber(args[1])
        if requested then
            requested = math.max(1, math.min(requested, 1e19))
            Cheat.Config.FlySpeed = requested
        elseif not Cheat.Config.FlySpeed then
            Cheat.Config.FlySpeed = 70
        end
        ...
        if not hum or not root then
            output("⚠️ Персонаж не готов", Color3.fromRGB(255,255,0))
            return
        end
...
RunService.RenderStepped:Connect(function()
    ...
    local speed = Cheat.Config.FlySpeed or 70
